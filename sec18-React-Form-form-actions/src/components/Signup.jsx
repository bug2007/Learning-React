...
function signupAction(prevFormState, formData) {  // when this func is executed for the first time, prevFormState will simply be the initial {errors: null} 
    ...
    const acquisitionChannel = formData.getAll('acquisition');  // will return an arr of values in case multiple checkboxes get checked
    ...
    if (errors.length > 0) {
      return { errors, enteredValues: {email, password, confirmPassword, firstName, lastName, role, acquisitionChannel, terms} };   // same as { errors: errors}
    }
