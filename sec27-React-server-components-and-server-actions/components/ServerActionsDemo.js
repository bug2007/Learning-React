...
export default function ServerActionsDemo() {
  async function saveUserAction(formData) {
    'use server'; // this server action is like a form action but it will be executed on the server. must use that async above. cant define server actions inside client components, but can use them inside client components by exporting the server action from somewhere else. 
